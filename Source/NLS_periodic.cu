...
void Initial_conditions(int N, real* x1,real* x2,real* x3, real* x4){
	...
	FS
		x1[j]=0.31379418035549456+1e-6*sin(j);
...
int main (int argc, char *argv[])
{
	...
	if(argc!=8){
		//       0  1  2   3     4  5        6               7
		printf("%s N L Timesteps g CF Period_Timesteps dt_Fraction\n N - Fourier modes; L - length; g - parameter; CF - control file (yes - 1/ no -0); Period_Timestep - timesteps in one period; dt_Fraction - dt on Monodromy matrix integration=dt/dt_Fraction.\n",argv[0]);
		return 0;
	}

	N=atoi(argv[1]);		//for Domain
	M=(int)floor(N/2)+1;	//for Image
	int timesteps=atoi(argv[3]);
	real L=atof(argv[2]);
	real g=atof(argv[4]);
	int CFile=atoi(argv[5]);
	int timestepsMonodromy=atof(argv[6]);
	real dt_coefficient=atof(argv[7]);
	...
	if(CFile==1){
		...
		read_control_file(M, x1_Re, x1_Im, x2_Re, x2_Im, x3_Re, x3_Im, x4_Re, x4_Im);
		printf(" done\n");
	}
...
//init Fourier modes
	...
	write_control_file(M, x1_Re, x1_Im, x2_Re, x2_Im, x3_Re, x3_Im, x4_Re, x4_Im);
