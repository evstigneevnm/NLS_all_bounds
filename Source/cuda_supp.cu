...
void device_allocate_real(int size, real *&m_device){
    
    cudaError_t cuerr=cudaMalloc((void**)m_device, sizeof(real)*size);
    if (cuerr != cudaSuccess)
    {
        fprintf(stderr, "Cannot allocate device array because: %s\n",
        cudaGetErrorString(cuerr));
...
void device_host_real_cpy(real* device, real* host, int size){
    ...
    cudaError_t cuerr=cudaMemcpy(device, host, sizeof(real)*size, cudaMemcpyHostToDevice);
    if (cuerr != cudaSuccess)
    {
...
void host_device_real_cpy(real* host, real* device, int size){
    cudaError_t cuerr=cudaMemcpy(host, device, sizeof(real)*size, cudaMemcpyDeviceToHost);
    if (cuerr != cudaSuccess)
    {
